...
resource "aws_s3_bucket" "artifact_n_log_bucket" {
    bucket = "my-we3-c02-artifact-and-logs-bucket-for-code-pipeline" # Change to your desired bucket name
    # region = "ap-south-1"
...
resource "null_resource" "empty_bucket" {
  provisioner "local-exec" {
    command = "aws s3 rm s3://my-we3-c02-artifact-and-logs-bucket-for-code-pipeline --recursive"
  }
...
resource "null_resource" "delete_bucket" {
  ...
  provisioner "local-exec" {
    command = "aws s3api delete-bucket --bucket my-we3-c02-artifact-and-logs-bucket-for-code-pipeline"
  }
